...
def precision_recall_curve_plot(y_true, prob_MP, path, fold):
    ...
    plt.show()
    plt.savefig(Path(path).joinpath("Pixelwise_PR_AUC_curve_fold_"+fold+".png"))
    return PR_optimal_thr


def ROC_plot(y_true, prob_MP, path, fold):
    ...
    plt.show()
    plt.savefig(Path(path).joinpath("Pixelwise_ROC_AUC_curve_fold_"+fold+".png"))
    return ROC_optimal_threshold


def get_thr(pred_images, GT_images, res_path, fold, y_info_path):
    ...
    df=pd.DataFrame(df)
    df.to_csv(Path(y_info_path).joinpath("Ytrue_Yprob_pixelwise_fold_"+fold+".csv"), index=False)
    
    pr_thr=precision_recall_curve_plot(y_true, prob_MP, res_path, fold)
    roc_thr=ROC_plot(y_true, prob_MP, res_path, fold)
...
def get_eval_metrics(pred_images, GT_images, res_path, fold, pr_thr, roc_thr):
    ...
    file = open(Path(res_path).joinpath("Fold_"+fold+"_evaluation_metrics.txt"), "w")
    ...
    df.to_csv(Path(res_path).joinpath("Pixelwise_Model_evaluation_result_fold_"+fold+".csv"), index=False)
    return avg_iou, avg_DC, pixel_acc, PC, recall, SP, F1


def getROC_curve(files, path):
    ...
    plt.legend(loc="lower right")
    plt.show()
    plt.savefig(Path(path).joinpath("9-fold Cross Validation ROC curve.png"), bbox_inches='tight', dpi=300)
    
    
def getPR_curve(files, path):
    ...
    plt.show()
    plt.savefig(Path(path).joinpath("9-fold Cross Validation PR curve.png"), bbox_inches='tight', dpi=300)
