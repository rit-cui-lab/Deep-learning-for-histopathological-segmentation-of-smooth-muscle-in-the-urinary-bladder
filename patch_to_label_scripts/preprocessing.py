...
def newdirs(config):
    ...
    config.train_patch_path_cyst = create_dir("cystectomy", config.train_patch_path, del_val = "False")
    config.train_patch_path_tur = create_dir("tur", config.train_patch_path, del_val = "False")
    ...
    config.y_info_path = create_dir("y_info", config.result_path, del_val = "False")
...
def get_patches(config, images, GT_imgs, patch_path, train_idx, img_type):
    
    # Initialize variables to store patch information
    config.mp_patches=0; config.nmp_patches=0; config.patches_name=[]; config.patches_label=[]
    ...
    for i in range(len(train_idx)):
        ...
        if (img_fname[2] == GT_fname[2] and img_fname[3] == GT_fname[3]):
            ...
            GT[GT == 128] = 0; GT = GT/255
            x=0
            while(x<=GT.shape[0]-config.patch_size):
                y=0
                while(y<=GT.shape[1]-config.patch_size):
                    patch_sum = np.sum(GT[x:x+config.patch_size,y:y+config.patch_size])
                    if (patch_sum == config.patch_size*config.patch_size*3):
                        patch=img[x:x+config.patch_size,y:y+config.patch_size,:]
                        patch=cv.cvtColor(patch,cv.COLOR_RGB2BGR)
                        number=get_patchNum(config.mp_patches)
                        patch_fname='_'
                        patch_fname=patch_fname.join(GT_fname)+"_MP_"+number+".png"
                        patch_fname=Path(patch_path).joinpath(patch_fname).as_posix()
                        cv.imwrite(patch_fname,patch)
                        config.patches_name.append(patch_fname);config.patches_label.append("MP")
                        config.mp_patches+=1
                        y+=config.patch_size
                        ...
                        patch=img[x:x+config.patch_size,y:y+config.patch_size,:]
                        patch=cv.cvtColor(patch,cv.COLOR_RGB2BGR)
                        number=get_patchNum(config.nmp_patches)
                        patch_fname='_'
                        patch_fname=patch_fname.join(GT_fname)+"_NMP_"+number+".png"
                        patch_fname=Path(patch_path).joinpath(patch_fname).as_posix()
                        cv.imwrite(patch_fname,patch)
                        config.patches_name.append(patch_fname);config.patches_label.append("NMP")
                        config.nmp_patches+=1
                        y+=config.patch_size
    ...
    patch_dict={'Patch_name': config.patches_name,'Label': config.patches_label}
    df=pd.DataFrame(patch_dict)
    df.to_csv(config.train_patch_path.joinpath(img_type+'_patch_details_with_labels.csv'),index=False)
    print ("Completed writing the csv file")
    return config
